...
# 39. &get_kl_score subroutine to determine KL-divergence score for given gene pair
# based on their PSSM. 
                                                                                        
sub get_kl_score($$$) {
  my($fac_A, $fac_B, $PSSMS) = @_;

  my(%p_hash) = %{ $PSSMS };		               # Get PSSM matrix.

  my($fac_A, $fac_B) = sort { scalar @{ $p_hash{$fac_A}{'A'} } <=>
    			      scalar @{ $p_hash{$fac_B}{'A'} } } ($fac_A, $fac_B);  # Order TF names by PSSM length.  

  my($l_1) = scalar @{ $p_hash{$fac_A}{'A'} }; 	       # Get length of shorter PSSM.
  my($l_2) = scalar @{ $p_hash{$fac_B}{'A'} }; 	       # Get length of longer PSSM.

  my(@bases) = ('A','C','T','G');
  my($kl_score);				       # Initialize KL-divergence score.
  my(@min_sum)=();				       # Initialize summa container for iteration.
  ...
  my($si) = int($l_1/2);			       # Initialize sliding for short motif.
  if ($l_1-$si<4) { $si = $l_1-4; }		       # If initial overlap is not at least 4, make it 4.

  my($inc)=-1; my($li)=0;			       # Set increment and long motif index for step I.
  my($iter) = $l_1-$si;			               # Overlap between the motif pair.
  ...
  while($si!=$li) { 
    ...
    for (my $i=0; $i<$iter; $i++) {  
      foreach my $b (@bases) {
        my($q) = $p_hash{$fac_A}{$b}[$si+$i];   
        my($p) = $p_hash{$fac_B}{$b}[$li+$i]; 
    ...
    push @min_sum, $sum1;			       # Collect calculated KL-divergence score for iteration.
  }
  ...
  my($inc)=1; my($si)=0;		               # Set increment and short motif index for step II.
  my($iter) = $l_1;				       # Overlap between the motif pair.

  for (my $m=0; $m<=($l_2-$l_1); $m++) {
    ...
    $li=$li+$inc;
    push @min_sum, $sum2;			       # Collect calculated KL-divergence score for iteration.
  }
  ...
  my($inc)=1; my($si)=0;		               # Set increment and short motif index for step II.
  my($iter) = $l_2-$li;			               # Overlap between the motif pair.
  ...
  while (($l_2-3)!=$li) {
    ...
    push @min_sum, $sum3;			       # Collect calculated KL-divergence score for iteration.
  }
  return (sort {$a <=> $b} @min_sum)[0];	       # Return KL-divergence score.
}
