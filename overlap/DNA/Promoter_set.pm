  ...
  foreach my $prom_name (keys %{ $self->{factors_list} }) {
    ...
    for (my $i=0; $i<@tf_list; $i++) {			        # Iterate through all TF pairs in promoter.
      ...
      for (my $j=$i+1; $j<@tf_list; $j++) {
        ...
        if ($fac_A ne $fac_B) {					 # Eliminate same TF-TF pairs.       
          if (exists $self->{tf_pair}{$pair}) {	                 # If pair in promoter already exists.
            $self->{tf_pair}{$pair}++;

            if (!defined $self->{dist_prom}{$pair}{$prom_name}) {
              $self->{dist_prom}{$pair}{$prom_name}=1;           # Mark distinct promoter. 
            }
          }
          else { 
            $self->{tf_pair}{$pair}=1; 			         # Initialize TF pair.
            $self->{dist_prom}{$pair}{$prom_name}=1;             # Mark distinct promoter. 
          }


          if ($overlap==1) {					 # If overlapping TF pair.
            if (exists $self->{tf_overlap}{$pair}) {	         # If pair in promoter already exists.
              $self->{tf_overlap}{$pair}++;

              if (!defined $self->{OL_prom}{$pair}{$prom_name}) {
                $self->{OL_prom}{$pair}{$prom_name}=1;           # Mark distinct promoter. 
              }
            }
            else { 
              $self->{tf_overlap}{$pair}=1; 			 # Initialize TF pair.
              $self->{OL_prom}{$pair}{$prom_name}=1;             # Mark distinct promoter. 
            }
          }
        }  
