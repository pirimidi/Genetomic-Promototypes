...
sub overlap() {
  ...
  foreach my $TF1 (@tf_list) {
    my($factor_1) = $TF1->{factor};			         # Get primary TF data.
    my($offset_1) = $TF1->{offset};

    foreach my $TF2 (@tf_list) {
      my($factor_2) = $TF2->{factor};			         # Get secondary TF data.
      my($offset_2) = $TF2->{offset};

      if (!( $factor_1 eq $factor_2 && 
                        $offset_1 == $offset_2 )) {  		# If not the same TF.
        my($A_B);

        my($m_A_start, $m_A_end); 
        my($m_B_start, $m_B_end);

        if ($offset_1 <= $offset_2) {				 # If expected TF order.
          $m_A_start = $TF1->{motif_start};			 # Assign TF1->motif_A &
          $m_B_start = $TF2->{motif_start};			 # assign TF2->motif_B.

          $m_A_end = $TF1->{motif_end};
          $m_B_end = $TF2->{motif_end};
          
          $A_B=1;
        }
        else {							 # If not: reverse order.
          $m_A_start = $TF2->{motif_start};			 # Assign TF1->motif_B &
          $m_B_start = $TF1->{motif_start};			 # assign TF2->motif_A.

          $m_A_end = $TF2->{motif_end};
          $m_B_end = $TF1->{motif_end};
        }
        ...
        if (($strand eq 'W' && $m_B_start <= $range) || 
          ...
          if ($A_B == 1) { $tf_pair = new DNA::Overlap($TF1, $TF2); }
          else 		 { $tf_pair = new DNA::Overlap($TF2, $TF1); }

          my($same) = &DNA::Func::overlap_dup_eliminate($tf_pair, @overlaps);
