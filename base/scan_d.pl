...
# promoter sequence for kmer occurences with a given sliding 
# window size and iteration gap, and finally constructs an 
# output in a textfile with the co-occuring permuted kmer 
# sequences in Perl.
#
...
if (@ARGV<2) {
  die "\n--> Error: not correct input!\n--> Usage: perl scan_d.pl gene config_file\n\n";
}
...
my(@FIELD) = (DNA::IO::FIELD1, DNA::IO::FIELD3, "Out_dir"); 
my(@TYPE) = (DNA::IO::TYPE1, DNA::IO::TYPE3); 
              
my(%valid_data) = &DNA::IO::evaluate($config_file, \@FIELD, \@TYPE);  # Evaluate input validity.
              
#------------------------- Valid IO parameter assignment ------------------------------

print "\n--> Process started - scan_d.pl";		     # Process start.
...
my(%seq_count) = $mutants->scan_d();			     # Document occurences in a hash.
my(%finalized) = $mutants->linear_filter(%seq_count);	     # Filter out redundant occurences in a hash.
$mutants->perm_scan(\%finalized, 0);			     # Generate permuted gene sequence.
