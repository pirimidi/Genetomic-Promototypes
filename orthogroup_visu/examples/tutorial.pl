...

{
	...
	$::MW->WidgetDemo(-name => $name, -text =>
$::DEMO_DESCRIPTION{$name});
    }
}
...
demonstration.  Once the demonstration window appears, you can click the ",
'', 'See Code', 'bold', " button to see the Perl/Tk code that created the
demonstration.  If you wish, you can edit the code and click the ", '',
"Rerun Demo", 'bold', " button in the code window to reinvoke the
...
demos 'Sample Perl Mega-Widgets', (
    ...
    'trace2'       => 'Derived   Tk::TraceText - Text contents defined by a
traced variable',
);
...
sub AUTOLOAD {
	...
	$DEMO_FILE = "$widget_lib/${demo}.pl" if -f
"$widget_lib/${demo}.pl";
	do $DEMO_FILE;
	warn $@ if $@;
    }
...
contributed demonstrations.  If no directory name is specified when widget
is
